...
reporter = Reporter(loss_logger, acc_logger, wgan_logger, f1_logger)

for e in range(EPOCHS):
    # Train session
    for i, ((fmri, img, label_idx), (img_u, label_idx_u)) in enumerate(zip(ld, ld_u)):
        ...
        for _ in range(d_epoch_steps):
            ...
            ld_real = netD(img_pack, f_pack, l_pack_idx).view(-1)
            fake_img = netG(zz, l_pack_idx, f_pack)
            ld_fake = netD(fake_img, f_pack, l_pack_idx).view(-1)

            # gp = model_utils.gradient_penalty(netD, fy_p, ly_p_idx, img, fake_img, dev)
            gp = model_utils.gradient_penalty(netD, f_pack, l_pack_idx, img_pack, fake_img, dev)
        ...
        # Validate stuff
        for (fmri_val, img_val, label_idx_val), (img_u_val, label_idx_u_val) in zip(ld_val, ld_u_val):
            non_img_model.eval()
            img_model.eval()
            ...
            if i == 0 and (e + 1) % export_gen_img_every == 0:
                zz = torch.randn(label_idx_val.shape[0], z_dim, 1, 1, device=dev)
                fake_img = netG(zz, ly_p_idx, fy_p)
...
acc_logger.export_file(os.path.join(LOGGER_PATH, "acc.lggr"))
