...
class EEGNet_AE_CNN_gray(nn.Module):
    """
    ...
    def __init__(self, in_channel, samples, kern_len, F1, F2, D, output_size, dropout_type='2D', dropout_rate=0.5,
        ...
        """
        super(EEGNet_AE_CNN_gray, self).__init__()
        self.is_debug = False
        ################# ENCODER ####################
        ...
        self.latent_fc = nn.Sequential(
            ConstraintLinear(in_features=F2 * (samples // 32), out_features=256, weight_max_lim=norm_rate),
#             nn.Softmax(dim=1)
        )
        ...
        self.dconv1 = nn.Sequential( nn.ConvTranspose2d(in_channels=16,out_channels=8, dilation=1, output_padding=0,  kernel_size=4,stride=2) ,  self.activation) 
        self.dconv2 = nn.Sequential( nn.ConvTranspose2d(in_channels=8,out_channels=5,  dilation=1, output_padding=0,  kernel_size=4,stride=2),   self.activation)
        self.dconv3 = nn.Sequential( nn.ConvTranspose2d(in_channels=5, out_channels=3,  dilation=1, output_padding=0,  kernel_size=4,stride=2),   self.activation)
        self.dconv4 = nn.Sequential( nn.ConvTranspose2d(in_channels=3, out_channels=1,  dilation=1, output_padding=0,  kernel_size=3,stride=1),   self.activation )
