...
class Classification_Training():
    def __init__( self , model_obj , learning_rate=0.001, criterion=nn.CrossEntropyLoss() , env="" , visdom_update = False ):
        ...
        # visdom config
        self.visdom_update = visdom_update
        
        if self.visdom_update :
            visdom_cfg = {"server": "192.168.11.12", "port": 8097}
            self.viz        = Visdom('http://' + visdom_cfg["server"], port = visdom_cfg["port"], env=f"{env[0]}-{env[1]}", log_to_filename=f"save/visdom_log/{env[0]}-{env[1]}.json")
            self.plot_name = ["Losses", "Accuracy"]
            self.viz.line([[0.,0.]], [0], win=self.plot_name[0], opts=dict(title=self.plot_name[0], legend=['train', 'validate']))
            self.viz.line([[0.,0.]], [0], win=self.plot_name[1], opts=dict(title=self.plot_name[1], legend=['train', 'validate']))
        
        print(f'The model {type(self.MODEL).__name__} has {self.count_parameters():,} trainable parameters')
        # print(self.MODEL)
    ...
    def do_training( self, train_iterator_in, val_iterator_in,  n_epochs = 10, scheduler=None  ):
        ...
        for epoch in range(n_epochs):
            ...
            ## send data to visdom
            if self.visdom_update :
                data_plot = { "1" : train_loss, "2": val_loss }
                self.visdom_double_line(data_plot, epoch, title=self.plot_name[0])

                
            if self.visdom_update :
                data_plot = { "1" : train_acc, "2": val_acc }
                self.visdom_double_line(data_plot, epoch, title=self.plot_name[1])
    ...
    def visdom_double_line(self, data_plot_dict, x_step,  title=""):
        self.viz.line([ [data_plot_dict["1"], data_plot_dict["2"]] ], [x_step], win=title, update='append')
