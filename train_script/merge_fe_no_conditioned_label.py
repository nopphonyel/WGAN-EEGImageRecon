...
netD = wgans_experimental.DiscriminatorNoLabel(ngpu=1, num_classes=num_class, img_channel=1, latent_size=200).to(dev)
    ...
    for e in range(EPOCHS):
        ...
        for i, (fmri, img, label_idx) in enumerate(ld):
            ...
            for _ in range(d_epoch_steps):
                ...
                ld_real = netD(img, fy_p).view(-1)
                fake_img = netG(zz, fy_p)
                ld_fake = netD(fake_img, fy_p).view(-1)

                gp = model_utils.gradient_penalty_no_label(netD, fy_p, img, fake_img, dev)
