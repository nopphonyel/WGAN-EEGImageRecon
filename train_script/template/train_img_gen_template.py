...
for epoch in range(num_epochs):
    for i, (img, label) in enumerate(up_ld):
        ...
        # Export image stuff
        if epoch % (num_epochs // 100) == 1 and i == 1:
            netG.eval()
            netIE.eval()
            up_ds.set_test()
            ...
            sys.stdout.write("\rExporting model")
            save_model(netIE, MODEL_PATH, "%d_netIE.pth" % epoch)
            save_model(netD, MODEL_PATH, "%d_netD.pth" % epoch)
            save_model(netG, MODEL_PATH, "%d_netG.pth" % epoch)
